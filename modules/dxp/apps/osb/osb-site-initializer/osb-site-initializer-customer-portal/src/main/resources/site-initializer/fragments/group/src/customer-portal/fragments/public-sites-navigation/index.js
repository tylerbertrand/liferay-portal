...
const emailAddress = themeDisplay.getUserEmailAddress();
...
const uiAvatarURL = `https://ui-avatars.com/api/${getInitials()}/128/0B5FFF/FFFFFF/2/0.33/true/true/true`;

const pathFormatted = `${gravatarAPI}/${emailAddressMD5}?d=${encodeURIComponent(
	uiAvatarURL
)}`;

const avatarContent = document.querySelector('.user-avatar > img');

if (avatarContent) {
	avatarContent.setAttribute('src', pathFormatted);
}
